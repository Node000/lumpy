...
var glue_swell_delay := 0.2
var glue_swell_scale := 6      # visual scale after swell (must equal collision factor)
var glue_swell_anim_time := 4
var glue_swell_collision_factor := 5  # final collision size after swell (must equal visual)
var glue_fly_timeout := 6.0
var glue_outside_margin := 90.0

# Liquid visual
var glue_visual_wobble := 0
var glue_visual_speed := 0
var glue_visual_flow_speed := 0
var glue_visual_distortion := 0
var glue_visual_edge_softness := 0
var glue_visual_stretch := 0
var player_body_color := Color(0.984, 0.965, 0.953, 1.0)
